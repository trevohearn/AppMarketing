...
def createStats(df, cluster):
    ...
    ymean = means[cluster]
    if (ymean > m): #cluster of new app is bigger
        count = 0
        while(ymean > m):
            ymean = ymean - deviation
            count += 1
        print('Your cluster is {} deviation(s) above the mean'.format(count))
    elif (ymean < m):
        count = 0
        while (ymean < m):
            ymean += deviation
            count += 1
        print('Your cluster is {} deviation(s) below the mean'.format(count))
    else:
        print('Your cluster is the mean')
...
for num, d in enumerate(dfs):
    ...
    appmeans.append(createStats(d, clusters[num]))
for num, t in enumerate(topGenres):
    print('{}'.format(t))
    print('Your mean: {}'.format(appmeans[num]))
    print('------------------------------')
